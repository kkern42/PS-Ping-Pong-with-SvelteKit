...
<html lang="en">
	<head>
		...
		<style>
			...
			.pong-container {
				padding: 4rem 8.5rem 0.5rem 8.5rem;
			}
	...
	<div class="pong-page">
		<div class="pong-container">
			...
			<p style="text-align: center; font-size: 10px;">
				Use the W and S keys to move the left paddle and the up and down arrows to move the right.
			</p>
			<pre><div style="padding: 0 1rem 0 1rem;" class="score">Score: 000000</div></pre>
			<div class="board">
				<div class="ball">
					<div class="ball_effect" />
				</div>
				<div class="paddle_1 paddle" />
				<div class="paddle_2 paddle" />
			</div>
		</div>
		...
		<div id="myModal" class="modal">
			<div class="modal-content">
				...
				<div class="inputs">
					<input
						id="scoreinput"
						style="width: 20%; margin-right: .5rem;"
	...
	<script>
		...
		(scrollLeft = window.pageXOffset || document.documentElement.scrollLeft),
			// not allowing scroll from track pad
		...
		document.addEventListener('keydown', (e) => {
			...
			if (gameState == 'play') {
				if (e.key == 'w') {
					paddle_1.style.top =
						Math.max(board_coord.top, paddle_1_coord.top - window.innerHeight * ballVelo) + 'px';
					paddle_1_coord = paddle_1.getBoundingClientRect();
				}
				if (e.key == 's') {
					paddle_1.style.top =
						Math.min(
							...
							paddle_1_coord.top + window.innerHeight * ballVelo
						) + 'px';
					paddle_1_coord = paddle_1.getBoundingClientRect();
				}
				if (e.key == 'ArrowUp') {
					paddle_2.style.top =
						Math.max(board_coord.top, paddle_2_coord.top - window.innerHeight * ballVelo) + 'px';
					paddle_2_coord = paddle_2.getBoundingClientRect();
				}
				if (e.key == 'ArrowDown') {
					paddle_2.style.top =
						Math.min(
							board_coord.bottom - paddle_common.height,
							paddle_2_coord.top + window.innerHeight * ballVelo
						) + 'px';
					paddle_2_coord = paddle_2.getBoundingClientRect();
				}
			}
		...
		const getLeaderBoard = async () => {
			await fetch(
				'https://pspptofirebase.azurewebsites.net/api/AccessFirebase?collection=LeaderBoard&code=WFVEZmP8cqOe1Fyt7Q6Po07lpvhO6eRXtG9DCziwvbSOAzFuiHLimw=='
			)
		...
		const submitScore = async () => {
			...
			await fetch(
				'https://pspptofirebase.azurewebsites.net/api/AccessFirebase?collection=LeaderBoard&score=' +
					newScore +
					'&name=' +
					username +
